...
The $MOIM$ step aims to minimize redundant processing and memory usage. It matches differences between frames and avoids reprocessing frames with similar information. This optimization step is crucial for achieving high compression ratios and efficient processing.

Ideally we proposed the utilization of a global difference matrix called $K_{global}$ where we track against a set of values used on the same matrix as it was a memory registry. This matrix will be used to compute the difference between frames and then evaluate if the difference threshold is small enough to be considered part of the same cluster.

Huffman coding and Discrete Cosine Transform (DCT) techniques can be applied to the differences between frames to achieve high compression ratios. This step also involves managing memory efficiently to minimize the computational overhead of processing differences. (see Annex \ref{appendix:a} for more information)

In collaboration with the first step of this pipeline, $MOIM$ ensures that the encoding algorithm can achieve high compression ratios while maintaining image quality.
...
The inter-frame encoding algorithm can be applied to a wide range of applications, including video compression, medical imaging, and remote sensing. By exploiting temporal redundancy between consecutive frames, inter-frame encoding can achieve high compression ratios while maintaining image quality.
